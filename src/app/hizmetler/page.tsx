            ...
            {kurumsal.map((svc, i) => (
              <motion.div key={svc.slug} variants={scaleIn} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i * 0.08}>
                <Link href={`/hizmetler/${svc.slug}`}
                  className="group block relative rounded-2xl overflow-hidden card-image-zoom bg-white border border-black/[0.06] hover:border-brand/25 transition-all duration-500 hover:-translate-y-2 shadow-sm hover:shadow-lg">
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-sm text-black/40 leading-relaxed mb-4">{svc.description}</p>
